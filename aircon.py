...
from lib.ac_weekly import estimate_weekly_cost
...
def aircon_portal():
    ...
    weekly = controller.get_weekly_config()
    rate_per_hour = current_app.config["AIRCON_RATE_PER_HOUR"]
    return render_template(
        "aircon_portal.html",
        initial_data={
            ...
            "weekly": weekly,
            "weekly_estimate": estimate_weekly_cost(weekly, rate_per_hour),
            "weekly_status": controller.get_weekly_status(),
...
def aircon_weekly_api():
    ...
    payload = request.get_json(silent=True)
    if not isinstance(payload, dict):
        return _json_error(AcControlError("Send the weekly schedule as JSON."))
    try:
        weekly = controller.save_weekly_config(payload)
        return jsonify(
            {
                "ok": True,
                "message": "Weekly automation saved.",
                "weekly": weekly,
                "weekly_estimate": estimate_weekly_cost(weekly, rate_per_hour),
                "weekly_status": controller.get_weekly_status(),
                "status": controller.get_status(),
...
@aircon_required
def aircon_history_api():
    controller = get_ac_controller(current_app)
    return jsonify({"ok": True, "history": controller.get_history()})
...
def aircon_stop_api():
    ...
    try:
        ...
        return jsonify(
            {
                "ok": True,
                "message": "Schedule stopped. Weekly automation is off." if weekly_disabled else "Schedule stopped.",
                "status": controller.get_status(),
                "statistics": controller.get_statistics(),
                "weekly": controller.get_weekly_config(),
                "weekly_status": controller.get_weekly_status(),
...
def register_routes(app):
    ...
    app.add_url_rule("/aircon/api/weekly", "aircon_weekly_api", aircon_weekly_api, methods=["GET", "POST"])
    app.add_url_rule("/aircon/api/history", "aircon_history_api", aircon_history_api)
